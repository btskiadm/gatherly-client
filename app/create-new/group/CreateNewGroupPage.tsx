...
import { GroupDetails, GroupDetailsRef } from "./components/GroupDetails";
...
const steps = ["Group details", "Payment details", "Review your order"];

export function CreateNewGroupPage(props: { disableCustomTheme?: boolean }) {
  const { data } = useMutation({
    mutationFn: createGroupMutationFn,
    onError: () => {
      toast.error("Wystąpił błąd w trakcie tworzenia grupy. Spróbuj ponownie poźniej.");
    },
    onSuccess: (data) => {
  ...
  const groupDetailsRef = React.useRef<GroupDetailsRef | null>(null);

  const [activeStep, setActiveStep] = React.useState(0);

  const handleNext = () => {
    if (activeStep === 0) {
      const groupDetailsData = groupDetailsRef.current?.save();
      console.dir({ groupDetailsData });
      ...
      >
        ...
        >
          ...
          {activeStep === steps.length ? (
            ...
            <React.Fragment>
              {activeStep === 0 && <GroupDetails ref={groupDetailsRef} />}
